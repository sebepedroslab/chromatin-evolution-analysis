...
# fraction of fully validated hits
xtab = t(table(gen_fusions$core_class_string_simple, gen_fusions$evidence_status)[class_string_sorted_nam,])
barplot(
  xtab, 
  horiz = T, las=1, col=rainbow(nrow(xtab), end = 0.9),
  main = sprintf("Evidence distribution\nn=%i", nrow(gen_fusions)),cex.names = 0.8)
legend("bottomright", fill=rainbow(nrow(xtab), end = 0.9), legend = rownames(xtab), bty = "n")

xtay = xtab[1,] / colSums(xtab) * 100
barplot(
  xtay, 
  horiz = T, las=1, col="green",
  names.arg = sprintf("%s %.1f", colnames(xtab), xtay),
  main = sprintf("Evidence fraction complete\nn=%i", nrow(gen_fusions)),cex.names = 0.8)
...
# non-unique table of fusions with their orthology (DO NOT USE!)
gen_fusions_wog = merge(gen_fusions, ort, by.x = "gene", by.y="gene", all.x=T, all.y=F)
write.table(
  gen_fusions_wog, 
  file = "results_TEfusions/gene_fusion_evidence.with_OG.csv" ,
  quote = F, sep = "\t", row.names = F)
  ...
  if (nrow(gen_fusions_i)>0){
    ...
    for (ogi in hgs_w_TE) {
      ...
      if (file.exists(sprintf("%s/euk.%s.seqs.iqtree.treefile", tree_fo, ogi))) {
        ...
        plot.phylo(gphy, font=1, type="fan", label.offset = 0.01, cex=text_size, edge.color = "gray",
    ...
    # SECOND
    # plot core trees with TE domain identity
    hgs_w_TE = unique(gen_fusions_i$homology_group)
    pdf(file = sprintf("results_TEfusions/tree_core_with_TEtype.%s.pdf", cli),width = 12, height = 12)
    for (ogi in hgs_w_TE) {
      ...
      if (file.exists(sprintf("%s/euk.%s.seqs.iqtree.treefile", tree_fo, ogi))) {
        ...
        rownames(gphy_fus) = gphy_fus$gene
        gphy_fus = gphy_fus[gphy$tip.label,]
        gphy_fus_tab = table(gphy_fus$TE_type)
        gphy_fus_cla = names(gphy_fus_tab)
        gphy_fus_col = rainbow(n=length(gphy_fus_cla), end = 0.8, v = 0.8)
        gphy_fus$TE_type[is.na(gphy_fus$TE_type) & !is.na(gphy_fus$core_class_string)] = "other"
        gphy_fus$TE_type[is.na(gphy_fus$TE_type) & is.na(gphy_fus$core_class_string)] = "none"
        ...
        plot.phylo(gphy, font=1, type="fan", label.offset = 0.01, cex=text_size, edge.color = "gray",
                   root.edge = T, show.tip.label = F, align.tip.label = F, underscore = T)
        # label tips        
        unlabeled_tips = which(gphy_fus$TE_type == "none")
        tiplabels(text=gphy$tip.label[unlabeled_tips], tip = unlabeled_tips, col = alpha("darkgray",0.7), bg = NA, frame = "none", cex=0.4)
        # plot classes per fusion
        for (fui in seq_along(gphy_fus_cla)) {
          gphy_fus_cla_i = gphy_fus_cla[fui]
          gphy_fus_col_i = gphy_fus_col[fui]
          tips_w_TE = which(gphy_fus$TE_type == gphy_fus_cla_i)
          tiplabels(pch = 19, col = gphy_fus_col_i, height = 4, cex = 0.7, tip = tips_w_TE)
          tiplabels(text=gphy$tip.label[tips_w_TE], tip = tips_w_TE, col = alpha(gphy_fus_col_i,0.8), bg = NA, frame = "none", cex=0.5)
        }
        # mark tips with full evidence
        ...
        title(
          main = sprintf("%s",ogi),
          sub =  sprintf("%i genes\nn=%i fusion(s) with RT domains\n(%i pass filters)", length(gphy$tip.label), sum(gphy_fus$TE_type!="none"), length(tips_w_evidence)))
        # add scale bar
        add.scale.bar(x=0, y=0)
...
#### Sum evidence: per OG and species ####

pdf("results_TEfusions/heatmap_TE_fusion_per_ogsps.pdf", height = 9, width = 5)
for (cli in list_gene_fam) {
...
pdf("results_TEfusions/heatmap_TE_fusion_per_og.pdf", height = 4, width = 9)
for (cli in list_gene_fam) {
  ...
  # add orthology group (only within said gene family)
  ort_i = ort[ort$family == cli,c("gene","orthogroup")]
  ### FIX: merge very similar orthogroups from Chromo.HG3:
  ort_i [ ort_i$orthogroup == "Chromo.HG3.2:NA", "orthogroup"] = "Chromo.HG3.m2/3/5:NA"
  ort_i [ ort_i$orthogroup == "Chromo.HG3.3:NA", "orthogroup"] = "Chromo.HG3.m2/3/5:NA"
  ort_i [ ort_i$orthogroup == "Chromo.HG3.5:NA", "orthogroup"] = "Chromo.HG3.m2/3/5:NA"
  ...
  gen_fusions_i$species_core = paste(gen_fusions_i$species, gen_fusions_i$orthogroup)
  gen_fusions_i = gen_fusions_i[order(gen_fusions_i$orthogroup, gen_fusions_i$species),]
  species_core_order = unique(gen_fusions_i$orthogroup)
  ...
  if (nrow(gen_fusions_i)>0) {
    ...
    # table TE classification
    tab = xtabs(formula = ~ orthogroup + TE_classtype ,
                data = gen_fusions_i, drop.unused.levels	= F)
    tam = matrix(tab, nrow = dim(tab)[1])
    rownames(tam) = rownames(tab)
    colnames(tam) = colnames(tab)
    ...
    gen_fusions_i_TEdoms_list = sort(unique(stringr::str_split(paste(unique(gen_fusions_i$TE_domain_string), collapse = ","), ",")[[1]]))
    for (dom in gen_fusions_i_TEdoms_list) {
      gen_fusions_j = gen_fusions_i[,c("orthogroup","TE_domain_string")]
      gen_fusions_j$domain = grepl(pattern = sprintf("\\b%s\\b", dom), gen_fusions_j$TE_domain_string) * 1
      tab = aggregate(gen_fusions_j$domain, by=list(gen_fusions_j$orthogroup), sum)
      # rownames(tab) = tab$Group.1
      # tab = tab[species_core_order,]
      tat = cbind(tat, tab$x)
      colnames(tat)[length(colnames(tat))] = dom
    ...
    # table evidence status
    tab = xtabs(formula = ~ orthogroup + evidence_status ,
                data = gen_fusions_i, drop.unused.levels	= F)
    tam = matrix(tab, nrow = dim(tab)[1])
    rownames(tam) = rownames(tab)
    colnames(tam) = colnames(tab)
    ...
    # table monexon
    tab = xtabs(formula = ~ orthogroup + structure_status ,
                data = gen_fusions_i, drop.unused.levels	= F)
    tam = matrix(tab, nrow = dim(tab)[1])
    rownames(tam) = rownames(tab)
    colnames(tam) = colnames(tab)
    ...
    # table species
    tab = xtabs(formula = ~ orthogroup + species ,
                data = gen_fusions_i, drop.unused.levels	= F)
    tam = matrix(tab, nrow = dim(tab)[1])
    rownames(tam) = rownames(tab)
    colnames(tam) = colnames(tab)
...
#### Sum evidence: per OG, using dots ####

pdf("results_TEfusions/heatmap_TE_fusion_per_og-dots.pdf", height = 3.5, width = 7)
...
for (cli in list_gene_fam) {
  ...
  if (nrow(gen_fusions_i)>0) {
    ...
    # table species
    tab = xtabs(formula = ~ species + orthogroup,
                data = gen_fusions_i, drop.unused.levels	= F)
    tae = xtabs(formula = ~ orthogroup + evidence_status + species,
                data = gen_fusions_i, drop.unused.levels	= F)
    tae = as.data.frame(tae)
    
    plot(NA,NA,xlim=c(0,30), ylim=c(0,15),main=cli, axes=F, xlab=NA, ylab=NA)
    for (ri in 1:ncol(tab)) {
      
      col = tab[,ri]
      col = col[col != 0]
      names_sps = if (is.null(names(col))) { rownames(tab) } else { names(col) }
      names_ogi = colnames(tab)[ri]
      vector_sizes = unlist(lapply(col, function(x) min(x,25)))^(1/2)*0.7
      vector_evcol_bool = unlist(lapply(names_sps, function(x) "contiguous assembly, complete expression" %in% gen_fusions_i[gen_fusions_i$species == x & gen_fusions_i$orthogroup == colnames(tab)[ri],"evidence_status" ] ))
      vector_evcol = rep("blue", length(vector_evcol_bool))
      vector_evcol [ !vector_evcol_bool ] = "slategray3"
      points(x=1:length(col),y=rep(15-ri, length(col)), cex=vector_sizes, col=alpha(vector_evcol,0.7), pch=19)
      text(x=1:length(col),y=rep(15-ri, length(col)), names_sps, col=alpha("slategray4",0.8), cex=0.3)
      text(x=0,y=15-ri, names_ogi, cex=0.4,adj=1)
      
    }
    legend_sizes = c(1,2,5,10,20)
    points(x=rep(30, length(legend_sizes)), y=1:length(legend_sizes),cex = legend_sizes^(1/2)*0.7 , col=alpha("gray", 0.7), pch=19)
    text(x=rep(30, length(legend_sizes)), y=1:length(legend_sizes),labels = legend_sizes, pch=19, col="darkgray", cex=0.4)
...
gen_fusions_i$species_core = paste(gen_fusions_i$species, gen_fusions_i$orthogroup)
gen_fusions_i = gen_fusions_i[order(gen_fusions_i$orthogroup, gen_fusions_i$species),]
species_core_order = rev(class_string_sorted_nam)



# cross-tabulations...
pdf("results_TEfusions/heatmap_TE_fusion_evidence_short.pdf", height = 10, width = 10)
if (nrow(gen_fusions_i)>0) {
  # col separation
  colsep = c()
  
  # table TE classification
  tab = xtabs(formula = ~ core_class_string_simple + TE_class ,
              data = gen_fusions_i, drop.unused.levels	= F)
  tam = matrix(tab, nrow = dim(tab)[1])
  rownames(tam) = rownames(tab)
  colnames(tam) = colnames(tab)
  ...
  gen_fusions_i_TEdoms_list = sort(unique(stringr::str_split(paste(unique(gen_fusions_i$TE_domain_string), collapse = ","), ",")[[1]]))
  for (dom in gen_fusions_i_TEdoms_list) {
    gen_fusions_j = gen_fusions_i[,c("core_class_string_simple","TE_domain_string")]
    gen_fusions_j$domain = grepl(pattern = sprintf("\\b%s\\b", dom), gen_fusions_j$TE_domain_string) * 1
    tab = aggregate(gen_fusions_j$domain, by=list(gen_fusions_j$core_class_string_simple), sum)
    # rownames(tab) = tab$Group.1
    # tab = tab[species_core_order,]
    tat = cbind(tat, tab$x)
    colnames(tat)[length(colnames(tat))] = dom
  ...
  # table evidence status
  tab = xtabs(formula = ~ core_class_string_simple + evidence_status ,
              data = gen_fusions_i, drop.unused.levels	= F)
  tam = matrix(tab, nrow = dim(tab)[1])
  rownames(tam) = rownames(tab)
  colnames(tam) = colnames(tab)
  ...
  # table monexon
  tab = xtabs(formula = ~ core_class_string_simple + structure_status ,
              data = gen_fusions_i, drop.unused.levels	= F)
  tam = matrix(tab, nrow = dim(tab)[1])
  rownames(tam) = rownames(tab)
  colnames(tam) = colnames(tab)
  ...
  pheatmap(tas,
           gaps_col = colsep,
           color = col_blue(20), breaks = seq(0,20,length.out = 21)-0.01,
           cellwidth = 8, cellheight = 4, na_col = "grey",
           cluster_rows = F, cluster_cols = F,
           labels_row = stringr::str_trunc(rownames(tas), width = 100),
           fontsize = 4.5,
           main = sprintf("All fusions", cli),
           border_color = "white", display_numbers = T, number_format = "%i")
}
dev.off()
