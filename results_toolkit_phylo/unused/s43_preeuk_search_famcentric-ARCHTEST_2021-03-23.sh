...
ouf="results_preeuk/trees_adhfamarch"
...
domains_list="ASF1_hist_chap DOT1 Hist_deacetyl MOZ_SAS SIR2 SET Acetyltransf_1 GNAT_acetyltr_2 CupinJmjC ING MBT PWWP SNF2_N Histone"
...
for i in $domains_list ; do
	...
	cd-hit -i ${inf}/arc.${i}.domains.fasta -o tmp.arc.fasta -c 1 1> /dev/null
	cat <(sed "s/>/>arc_/" tmp.arc.fasta) > ${ouf}/tmp.${i}.database.fasta
	sleep 2 && rm -f tmp.bac.fasta* tmp.arc.fasta*
	...
	for clu in $clu_list; do
		...
		if [ $(grep -c '>arc_' ${ouf}/euk.${i}.phy.${clu}.seqs.fasta) -gt 0 ] ; then
