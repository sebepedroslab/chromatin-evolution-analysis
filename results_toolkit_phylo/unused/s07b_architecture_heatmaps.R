...
for (fam in list_fams) {
  ...
  if (nrow(arq)>0) {
    ...
    # ard_wte = ard[grepl(list_te_doms_rexp, ard$domains),]
    # ard_wte = ard_wte[grepl(list_fam_doms_rexp, ard_wte$domains),]
    # 
    ...
    #   
    #   # plot architectures heatmap
    #   pdf(file = sprintf("%s/arc.%s.genes_with_TE.pdf", arqs_fo, fam),width = 13, height = 4)
