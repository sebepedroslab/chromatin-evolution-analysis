...
# subset network to the largest components, plot them separately, and store their coordinates
dict_pal_pos = dict() # dict to store original coordiantes
dicm_pal_pos = dict() # dict to store modified coordinates
for n,members in enumerate(pal_cc):

	if len(members) < 100 and len(members)>50:
		...
		# store coordiantes
		dict_pal_pos[n] = pal_pos
		dicm_pal_pos[n] = { pal_pos[i][0]+n for i in pal_pos }
...
pal_pos = nx.spring_layout(pal_ns,pos=dict_pal_pos, weight="bitscore", iterations=10000, seed=111)
# get list of edge weights
pal_weight = list(nx.get_edge_attributes(pal_n,'bitscore').values())
