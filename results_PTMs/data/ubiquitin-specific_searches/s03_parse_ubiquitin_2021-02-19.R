...
# input files
mod_fn = "2021MH000_Fabomonas-Ub.xlsx"
out_fn = str_remove(mod_fn, "\\.xlsx$")
his_list = c("H3","H4","H2A","H2B","macroH2A","H2AZ","H1")

# dictionary of modifications
dict_mods = data.frame(
    Rawmod =       c("Phospho", "Propy","3xMethyl","2xMethyl","Crotonyl","Acetyl","GG"),
    Modification = c("Pho",     "Me1",  "Me3",     "Me2",     "Cro",     "Ace",   "Ubi")
)

# read ref seqs
mod_ref = read_excel(mod_fn, sheet="peptides")
...
mod_sum = data.frame(
    ...
    mods_in_prot = mod_ref$`Positions in Master Proteins`
)
...
for (i in 1:nrow(mod_sum)) {
    ...
    pep_pos = str_split(mod_sum$mods_in_prot[i], " ")[[1]][2]
